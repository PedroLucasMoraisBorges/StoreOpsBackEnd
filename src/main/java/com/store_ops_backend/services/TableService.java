...
public class TableService {
    ...
    @Transactional
    public TableSessionResponseDTO closeSession(String companyId, String sessionId, String paymentMethodId) {
        TableSession session = findSessionOrThrow(companyId, sessionId);
        if (paymentMethodId != null && !paymentMethodId.isBlank()) {
            com.store_ops_backend.models.entities.PaymentMethods pm = paymentMethodRepository.findById(paymentMethodId)
                .orElseThrow(() -> new EntityNotFoundException("Forma de pagamento não encontrada"));
            session.closeWithPayment(pm);
        } else {
            session.close();
        }
    ...
    private void deductStock(String companyId, Product product, ProductVariant variant, BigDecimal quantity) {
        ...
        try {
            ...
            // Stock debit is best-effort; don't fail the item addition
        }
    }
    ...
    private TableSessionResponseDTO toSessionResponse(TableSession s) {
        ...
        return new TableSessionResponseDTO(
            ...
            items, s.getTotal(), pmId, pmName, s.getPaidAt()
        );
    }
}
